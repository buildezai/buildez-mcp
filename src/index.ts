...
interface BuildWebsiteResponse {
  success: boolean;
  liveUrl?: string;
  projectPath?: string;
  deploymentId?: string;
...
function generateCleanWebId(businessName: string): string {
  return businessName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
...
async function getAvailableWebId(baseWebId: string): Promise<string> {
  try {
    // Check if base webId is available
    const checkResponse = await apiRequest<CheckWebIdResponse>(
      `/api/check-webid?webId=${baseWebId}`,
      'GET',
      undefined,
      10000 // 10 second timeout for check
    );
    ...
    for (let i = 2; i <= 99; i++) {
      ...
      const retryCheck = await apiRequest<CheckWebIdResponse>(
        `/api/check-webid?webId=${numberedWebId}`,
        'GET',
        undefined,
        10000
      );
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'build_website': {
        const { businessName, description } = (args || {}) as unknown as BuildWebsiteInput;

        if (!businessName || !description) {
          throw new McpError(
            ErrorCode.InvalidParams,
            'Both businessName and description are required'
          );
        }
        ...
        const aiBuilderResponse = await apiRequest<AIBuilderResponse>(
          '/api/ai-builder',
          'POST',
          {
        ...
        const buildResponse = await apiRequest<BuildWebsiteResponse>(
          '/api/build-website',
          'POST',
          {
